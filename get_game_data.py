...
def find_all_games_paths(sorce):
    game_path = []
...
def get_name_from_path(paths, to_strip):
    ...
    for path in paths:
        ...
        new_dir_name = dir_name.reaplace(to_strip, "")
...
def main(sorce, target):
    cwd = os.getcwd()
    source_path = os.path.join(cwd, sorce)
...
if __name__ == "__main__":
    arg = sys.argv
    if len(arg) != 3:
        raise Exception("you most pass the source and target directory - only")
